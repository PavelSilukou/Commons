...
public class ArcToArcIntersectionReturnTests
{
	private static IEnumerable<TestCaseData> IsArcToArcIntersectTestsParameters()
	{
		...
		yield return new TestCaseData(args24).Returns(returns4);
		yield return new TestCaseData(args25).Returns(returns1);
		yield return new TestCaseData(args26).Returns(returns1);
	}
	
	[Test, TestCaseSource(nameof(IsArcToArcIntersectTestsParameters))]
	public (bool, Vector2[]) IsArcToArcIntersectTests(
		Vector2 arc1Center,
		Vector2 arc1Point, 
		float arc1AngleDeg,
		Vector2 arc2Center,
